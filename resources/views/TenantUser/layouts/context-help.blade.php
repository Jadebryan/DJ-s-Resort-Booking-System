...
<div class="relative"
    ...
    <button type="button"
            @click="open = !open"
            class="rounded-xl p-2 text-gray-500 transition hover:bg-white hover:text-teal-700 hover:shadow-sm"
            :aria-expanded="open"
            aria-label="{{ __('Page help') }}"
            title="{{ __('Helpful links for this page') }}">
    ...
    <div x-show="open"
        ...
        <div class="sticky top-0 flex items-start justify-between gap-2 border-b border-gray-100 bg-gradient-to-r from-teal-50/90 to-white px-3 py-2.5">
            <p class="text-sm font-semibold text-gray-900">{{ __('Helpful links') }}</p>
            <button type="button"
                    @click="showBannerAgain()"
                    class="shrink-0 text-[11px] font-medium text-teal-700 hover:text-teal-900"
