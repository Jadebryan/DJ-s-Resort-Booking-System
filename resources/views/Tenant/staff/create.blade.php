<x-tenant::app-layout>
    <x-slot name="header">
        <div class="leading-tight min-w-0">
            ...
            <h1 class="text-lg font-semibold text-gray-800 sm:text-xl mt-1">{{ __('Add staff member') }}</h1>
            <p class="text-[11px] text-gray-500 mt-0.5">{{ __('Invite someone who can log in to manage this resort.') }}</p>
        </div>
    </x-slot>
    ...
    <div class="w-full min-w-0 max-w-7xl py-8 sm:py-10">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 min-w-0">
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6 overflow-hidden">
                <form method="POST" action="{{ tenant_url('staff') }}" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <x-tenant::text-input id="password" name="password" type="password" class="mt-1 block w-full" required />
                    ...
                    <div>
                        ...
                        <x-tenant::text-input id="password_confirmation" name="password_confirmation" type="password" class="mt-1 block w-full" required />
                    </div>
                    ...
                    <div>
                        ...
                        <select id="role" name="role" class="mt-1 block w-full border-slate-300 bg-white text-slate-900 rounded-md shadow-sm focus:border-sky-500 focus:ring-sky-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200">
                            <option value="staff" {{ old('role', 'staff') === 'staff' ? 'selected' : '' }}>Staff — Can manage rooms and bookings; cannot change branding or manage staff</option>
                            <option value="admin" {{ old('role') === 'admin' ? 'selected' : '' }}>Owner / Admin — Full access including branding and staff</option>
                        </select>
                        <x-tenant::input-error :messages="$errors->get('role')" class="mt-1" />
                    </div>

                    <div class="flex items-center gap-4">
                        <x-tenant::primary-button>{{ __('Add staff member') }}</x-tenant::primary-button>
