<section>
    ...
    <form method="post" action="{{ tenant_url('profile') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-tenant::text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $user->name)" required autofocus autocomplete="name" constraint="personName" />
            <x-tenant::input-error class="mt-2" :messages="$errors->get('name')" />
        </div>

        <div>
            <x-tenant::input-label for="email" :value="__('Email')" />
            <x-tenant::text-input id="email" name="email" type="email" class="mt-1 block w-full" :value="old('email', $user->email)" required autocomplete="username" constraint="email" />
